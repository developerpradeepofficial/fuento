            ...
            switch (event.channel.label) {
                case 'fluento_chat_channel':
                    try {
                        ...
                        console.error('fluento_chat_channel', err);
                    }
                    break;
                case 'fluento_file_sharing_channel':
                    try {
                        ...
                        if (dataFile instanceof ArrayBuffer && dataFile.byteLength != 0) {
                            ...
                            if (dataFile instanceof Blob && dataFile.size != 0) {
                                blobToArrayBuffer(dataFile)
                                    ...
                                    .catch((error) => {
                                        console.error('fluento_file_sharing_channel', error);
                                    });
                            }
                        }
                    } catch (err) {
                        console.error('fluento_file_sharing_channel', err);
                    }
                    break;
            }
        };
...
function setTheme() {
    ...
    fluentoTheme.selectedIndex = lsSettings.theme;
    const theme = fluentoTheme.value;
    switch (theme) {
        case 'dark':
            // dark theme
...
function shareRoomByEmail() {
    Swal.fire({
        ...
        preConfirm: () => {
            ...
            const email = '';
            const emailSubject = `Please join our Fluento P2P Video Chat Meeting`;
...
function createChatDataChannel(peer_id) {
    chatDataChannels[peer_id] = peerConnections[peer_id].createDataChannel('fluento_chat_channel');
...
function createFileSharingDataChannel(peer_id) {
    fileDataChannels[peer_id] = peerConnections[peer_id].createDataChannel('fluento_file_sharing_channel');
    ...
    if (isImageURL(incomingFileInfo.file.fileName)) {
        ...
        reader.onload = (e) => {
            Swal.fire({
                ...
                imageAlt: 'fluento-file-img-download',
                showDenyButton: true,
        ...
        Swal.fire({
            allowOutsideClick: false,
            background: swBg,
            imageAlt: 'fluento-file-download',
            imageUrl: images.share,
            position: 'center',
            title: 'Received file',
            text: incomingFileInfo.file.fileName + ' size ' + bytesToSize(incomingFileInfo.file.fileSize),
...
function leaveFeedback() {
    Swal.fire({
        ...
        text: 'Do you want to rate your Fluento experience?',
        confirmButtonText: `Yes`,
        denyButtonText: `No`,
        showClass: { popup: 'animate__animated animate__fadeInDown' },
        hideClass: { popup: 'animate__animated animate__fadeOutUp' },
