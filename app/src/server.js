...
const hostUsersString = process.env.HOST_USERS || '[{"username": "Pradeep", "password": "password"}]';
...
app.post([`${apiBasePath}/meeting`], (req, res) => {
    ...
    if (!api.isAuthorized()) {
        log.debug('Fluento - Unauthorized', {
            header: req.headers,
            body: req.body,
        });
    ...
    log.debug('Fluento get meeting - Authorized', {
        header: req.headers,
        body: req.body,
        meeting: meetingURL,
    });
...
app.post([`${apiBasePath}/join`], (req, res) => {
    ...
    const api = new ServerApi(host, authorization, api_key_secret);
    if (!api.isAuthorized()) {
        log.debug('Fluento get join - Unauthorized', {
            header: req.headers,
            body: req.body,
        });
        return res.status(403).json({ error: 'Unauthorized!' });
    ...
    log.debug('Fluento get join - Authorized', {
        header: req.headers,
        body: req.body,
        join: joinURL,
    });
